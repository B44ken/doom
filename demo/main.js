...
setInterval(() => {
    const step = (dist) => {
        advance(player, dist);
        if (getMap(player.x, player.y) != " ") advance(player, -dist);
    };
    if (keys["w"]) step(0.05);
    if (keys["s"]) step(-0.05);
    ...
    while (getMap(ray.x, ray.y) == " ") {
        ...
        for (const enemy of enemies)
            if (distance(ray, enemy.point) < 0.25 && enemy.alive)
                return { ray, enemy };
        if (distance(pos, ray) > 15) return null;
    }
...
function render() {
    ...
    for (let x = 0; x < canvasWidth; x++) {
        ...
        for (let y = 0; y < canvasHeight; y++) {
            if (y < minHeight) setPixel32(x, y, 0xffe3a97d);
            if (y > minHeight && y < maxHeight) setPixel(x, y, color);
            if (y > maxHeight) setPixel32(x, y, 0xff42423a);
        }
    }
    return slices;
}
